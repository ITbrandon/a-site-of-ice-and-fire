...
import { forgotUsername } from '../server/api'
...
export async function action({request}: ActionFunctionArgs) {
    ...
    {
      try {
       const request = await forgotUsername(email)
       return request
      }
...
const error = useActionData()
  ...
  return (
    <>
    {error?.message === "Username sent to your email" ? <Confirmation title={"Username Sent"} message={"Check Your Inbox For your username"} link={true} /> : <div className="flex min-h-full flex-1 flex-col justify-center px-6 py-12 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-sm">
        ...
        <h2 className="mt-10 text-center text-2xl/9 font-bold tracking-tight text-gray-900">
          Enter email to recieve Username
        </h2>
